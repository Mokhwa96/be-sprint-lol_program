...
class LoL_char {
  ...
  String[] user_create() {
    ...
    String Unit_name = input.nextLine();
    ...
    return new String[]{Unit_name, ad, def, hp};
  ...
  int[] user_info_int(String[] info) {
    //TODO:
    // 같은 형식의 반환을 위해 배열 선언
    int[] intinfo = new int[3];
    intinfo[0] = Integer.parseInt(info[1]);
    intinfo[1] = Integer.parseInt(info[2]);
    intinfo[2] = Integer.parseInt(info[3]);
  ...
  void attack(int[] me_info_int, int[] enemy) {
    ...
    //TODO:
    System.out.println("------------------------------------");
    if  (enemy[2] <= me_info_int[0] / enemy[1]) {
